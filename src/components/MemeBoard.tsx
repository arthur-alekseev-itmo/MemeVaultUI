...
function Error({ code }: { code: string }) {
    ...
    return <div className="nothing-found-div">
        ...
        <div className="nothing-found-text">
            <span className="nothing-found">{t("Error loading images")}</span>
        </div>
        <div className="nothing-found-text">
            <span className="nothing-found">{t("Server returned ")} <a> {code} </a> </span>
        </div>
    </div>
}
...
export function MemeBoard() {
    ...
    const [allMemes, setAllMemes] = useState<meme[] | null | undefined>(null);
    ...
    const refresh = () => getAllMemes()
        .then(r => { setAllMemes(r.images); handleFilter("") })
        .catch(e => setCode(e))
    ...
    if (memes === undefined) {
        return <div style={{ height: "calc(100vh - 4rem)" }}> <Error code={code} /> </div>
    }
